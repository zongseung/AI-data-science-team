...
### 1.2 비전
AI 에이전트들이 각 팀(수집, 분석, 전망, 보고서)으로 구성되어 **데이터 사이언스 방법론**(EDA, 피처 엔지니어링, ML/DL 모델링, 백테스팅)을 적용하여 한국 주식시장 데이터를 수집-분석-예측하고, 보고서팀이 종합 결과를 리포트로 작성하며, 그 과정을 웹에서 캐릭터 애니메이션으로 시각화하고, 결과를 텔레그램으로 전달하는 시스템
...
#### FR-005: 웹 시각화
- PixiJS 8 기반 오피스 시뮬레이션 (논리 캔버스 960×640 px)
- 4개 팀 오피스: 수집팀, 분석팀, 예측팀, 보고서팀 (각 방 260×150 px)
- 캐릭터 스프라이트: 32×48 px 원본, scale 2.0 → 화면 64×96 px
- 캐릭터 애니메이션: 수집 중, EDA 중, 모델 학습 중, 백테스팅 중, 보고서 작성 중 등
- 실시간 작업 상태 표시 (WebSocket → CharacterState 전환)
...
### 3.4 보고서팀 (Report Team) ← NEW
```
보고서팀장 (Report Team Lead)
├── 종합 리포터 에이전트 (Comprehensive Reporter)
│   ├── 수집·분석·ML 전체 결과 종합
│   ├── LLM 기반 자연어 리포트 생성
│   └── 차트 이미지 생성 (matplotlib/plotly)
│
├── 투자 메모 작성 에이전트 (Investment Memo Writer)
│   ├── ML 예측 + 백테스트 → 투자 판단 요약
│   ├── 매수/매도 시그널 해석
│   └── 투자 의견 정리
│
├── 리스크 노트 작성 에이전트 (Risk Note Writer)
│   ├── VaR/CVaR/Monte Carlo 결과 경고 수준별 정리
│   ├── 리스크 시나리오 요약
│   └── 주의 종목 플래그
│
└── 편집장 에이전트 (Report Editor / Chief)
    ├── 종합 리포트 + 투자 메모 + 리스크 노트 최종 검토
    ├── 품질 검증 (수치 정합성, 논리 일관성)
    ├── 텔레그램 포맷 변환 & 발송
    └── 모델 성능 해석 포함
```
...
agent_tasks (에이전트 작업)
├── id: UUID
├── agent_type: VARCHAR (data_engineer, data_scientist, ml_engineer, report_writer)
├── agent_name: VARCHAR
├── task_type: VARCHAR
...
[보고서팀]  ──────────────────────────────────────────────────────────────
                                                                      │
 수집·분석·ML                                                          │
 전체 결과 수신  →  종합 리포트  →  투자 메모  →  리스크 노트  →  편집장 검토
                   작성            작성          작성          & 최종 승인
                                                                      │
                                                                      ▼
                                                              [텔레그램 전송]
...
11. 봇 → "ML 분석 완료! 보고서팀에 전달합니다."
12. [웹 시각화] 서류 배달 → 보고서팀

13. 보고서팀 → 종합 리포트 작성
    a) 종합 리포터: 수집·분석·ML 결과 종합 리포트 작성 (LLM)
    b) 투자 메모: ML 예측 + 백테스트 기반 투자 의견 메모
    c) 리스크 노트: VaR/CVaR 기반 리스크 경고 노트
    d) 편집장: 최종 검토 & 승인
...
- [ ] **백테스팅 프레임워크 구현**
- [ ] **MLflow 실험 추적 연동**
### Phase 4.5: 보고서팀 구현 (1주)
- [ ] **종합 리포터 에이전트 (수집·분석·ML 결과 종합 → LLM 리포트)**
- [ ] **투자 메모 작성 에이전트 (ML 예측 → 투자 의견 요약)**
- [ ] **리스크 노트 작성 에이전트 (VaR/CVaR → 경고 수준별 정리)**
- [ ] **편집장 에이전트 (최종 검토·품질 검증·텔레그램 발송)**
- [ ] 보고서팀 매니저 및 오케스트레이터 연동

### Phase 5: 웹 시각화 (2주)
- [ ] React + PixiJS 8 프로젝트 설정
- [ ] CANVAS_CONFIG 상수 정의 (960×640, 스프라이트 32×48, scale 2.0)
- [ ] 오피스 씬 구현 (4개 팀룸, 각 260×150 px, y=0/160/320/480)
- [ ] 캐릭터 스프라이트 시트 제작 (256×144 px, row 0~2)
- [ ] 캐릭터 애니메이션 & 파티클 구현
- [ ] 서류 배달 경로 애니메이션 (16×16 px 서류 스프라이트)
- [ ] WebSocket 실시간 연동 (CharacterState 전환)
...
├── 04-1_보고서팀/
│   └── report_team.md               ← 보고서팀 상세 설계 (종합리포터/투자메모/리스크노트/편집장)
├── 05_텔레그램_연동/
