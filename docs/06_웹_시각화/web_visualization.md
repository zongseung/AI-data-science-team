...
**Claw-Empire** 스타일 픽셀아트 탑뷰 오피스 시뮬레이션 + **Hyperbeat** 트레이딩 터미널 UI를 결합한 하이브리드 대시보드.

- AI 금융 데이터사이언스 팀의 작업 과정을 **실시간 픽셀 캐릭터 애니메이션**으로 시각화
- 4개 팀 방이 캔버스 전체를 꽉 채우며 동적 스케일링
- 오른쪽 사이드바는 Hyperbeat 트레이딩 터미널 스타일 데이터 패널

### 1.1 디자인 레퍼런스

| 레퍼런스 | 차용 요소 |
|---------|---------|
| **Claw-Empire** | 탑뷰 픽셀아트 오피스 룸, 바닥 타일, 픽셀 캐릭터 스프라이트, 말풍선, 배달 애니메이션 |
| **Hyperbeat** | `#060a10` 극암 배경, `#00c076`/`#ff3b69` 트레이딩 컬러, 모노스페이스 데이터 패널, 스캔라인 효과 |
...
--panel:       #0d1117
--border:      #1c2333
--green:       #00c076   (수집팀 / 양봉 / 성공)
--red:         #ff3b69   (리스크 / 음봉 / 오류)
--gold:        #f0b429   (보고서팀 / 강조)
--cyan:        #39d0d8   (분석팀)
--purple:      #a371f7   (ML팀)
--blue:        #388bfd   (대화형 요소)
