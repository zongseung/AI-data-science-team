...
────────────────────┼─────────────────────────┼──────────────
대기중              │ 문서 앞 대기              │ idle 상태
데이터 수신 중       │ 여러 서류 날아오는 이펙트  │ 팀별 결과 수신
종합 리포트 작성 중  │ 여러 색 문서 → 하나로 합침 │ 수집·분석·ML 종합
투자 메모 작성 중    │ 문서 + 매수/매도 파티클    │ 투자 의견 요약
리스크 노트 작성 중  │ 경고 삼각형 → 문서 변환    │ 리스크 경고 정리
편집장 검토 중       │ 빨간펜 체크 파티클         │ 최종 검토/교정
품질 검증 실패       │ ❗ 빨간 느낌표 + 되돌리기  │ 수정 필요
최종 승인           │ 금색 완료 파티클 ✓        │ 보고서 승인
텔레그램 전송 중     │ 텔레그램 아이콘 날아감     │ 결과 전송
전송 완료           │ ✅ 체크 + 반짝 파티클     │ 완료 축하
```
